...
const ShareModal = ({roomId,collaborators,creatorId,currentUserType}:ShareDocumentDialogProps) => {
  ...
  const shareDocumentsHandler = async ()=>{
  ...
  <DialogContent className='shad-dialog'>
    ...
    <div className='flex items-center gap-3 '>
       ...
       <UserTypeSelector userType={userType} setUserType={setUserType}/>
        ...
        <Button className='gradient-blue flex h-full gap-1 px-5' disabled={loading} type='submit' onClick={shareDocumentsHandler}>{loading? 'Sending...' : 'Invite'}</Button>
    </div>
...
<ul className='flex flex-col '>
    {collaborators.map((colaborator)=>(
        <Collaborator  key={colaborator.id} roomId={roomId} creatorId={creatorId} email={colaborator.email} collaborator={colaborator} user={user.info}/>
    ))}
</ul>
